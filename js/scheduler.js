...
export class Scheduler {
  constructor(playlist, options = {}) {
    ...
    this.groupIndex = 0;
    this.burstQueue = [];
    this.canRender = typeof options.canRender === "function"
      ? options.canRender
      : () => true;
  ...
  async init() {
    ...
    for (const item of this.playlist.items || []) {
      ...
      if (!firstEntry && entries.length) {
        firstEntry = cloneEntry(entries[0]);
      }
      // Accept the playlist's "one-shot" kind, keeping "once" as an alias.
      if (item.kind === "anchor") {
        this.anchors.push(...entries.filter(entry => this.canRender(entry)));
      } else if (item.kind === "one-shot" || item.kind === "once") {
        for (const entry of entries) {
          if (!this.canRender(entry)) {
            continue;
          }
        ...
        this.wildcardTemplates.push(...entries.filter(entry => this.canRender(entry)));
      }
    }
    ...
    const hasRenderable = this.anchors.length
      || this.onceQueue.length
      || this.wildcardTemplates.length
      || this.wildcardGroups.length;
    if (!hasRenderable && firstEntry) {
      this.fallbackEntry = firstEntry;
    }
  }
  ...
  #buildWildcardGroups() {
    ...
    const groups = tags.map(tag => ({
      tag,
      entries: this.wildcardTemplates.filter(entry => entry.tag === tag),
    }));
    const unlisted = this.wildcardTemplates.filter(entry => !listed.has(entry.tag));
    if (unlisted.length) {
      groups.push({ tag: null, entries: unlisted });
    }
  ...
  #burstConfig(tag) {
    const configs = this.playlist.policy?.group_burst;
    if (!tag || !configs || typeof configs !== "object" || Array.isArray(configs)) {
      return null;
    }
    ...
    if (typeof config.intro_template !== "string" || !config.intro_template.length) {
      return null;
    }
    return config;
  }

  #burstLimit(config, bagLength) {
    if (config.max_per_burst === null) {
      return bagLength;
    }
    const requested = Number(config.max_per_burst);
    if (Number.isFinite(requested) && requested >= 0) {
      return Math.min(Math.floor(requested), bagLength);
    }
    return Math.min(DEFAULT_BURST_CAP, bagLength);
  }

  #pickFromGroupBag(group, avoidKey = this.lastKey) {
    let pickIdx = group.bag.findIndex(entry => (
      this.#key(entry) !== group.lastKey
      && this.#key(entry) !== avoidKey
    ));
    if (pickIdx === -1) {
  ...
  #buildBurst(group, config) {
    const limit = this.#burstLimit(config, group.bag.length);
    const picks = [];
    let previousKey = this.lastKey;
    while (picks.length < limit && group.bag.length) {
      const pick = this.#pickFromGroupBag(group, previousKey);
      picks.push(pick);
      previousKey = this.#key(pick);
    }
    const dwell = group.entries[0]?.dwell_seconds
      || this.dwellSeconds
      || this.playlist.policy?.min_dwell_seconds
      || 5;
    const intro = {
  ...
  #pickWildcard() {
    if (this.wildcardGroups.length) {
      ...
      const burstConfig = this.#burstConfig(group.tag);
      if (burstConfig) {
        const burst = this.#buildBurst(group, burstConfig);
        const [pick, ...queued] = burst;
        this.burstQueue.push(...queued);
        this.groupIndex = (this.groupIndex + 1) % this.wildcardGroups.length;
        return pick;
      }
      const pick = this.#pickFromGroupBag(group);
      this.groupIndex = (this.groupIndex + 1) % this.wildcardGroups.length;
      return pick;
    }
  ...
  next() {
    this.position += 1;
    let pick = this.burstQueue.length
      ? this.burstQueue.shift()
      : (this.#pickAnchor() || this.#pickOnce() || this.#pickWildcard());
    if (!pick && this.fallbackEntry) {
      pick = cloneEntry(this.fallbackEntry);
    }
    if (!pick) {
      return null;
    }
  ...
  previewNext() {
    const snapshot = {
      ...
      burstQueue: this.burstQueue.map(cloneEntry),
    ...
    this.burstQueue = snapshot.burstQueue;
